...
const LoginForm = () => {
  ...
  const [password, setPassword] = React.useState('hashedpassword');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        welcomeText.textContent = `Welcome to the Trade AI Platform, ${data.data.user.firstName || data.data.user.email}!`;
        welcomeText.style.color = '#666';
        welcomeText.style.marginBottom = '16px';
        
        const companyText = document.createElement('p');
        companyText.textContent = `Company: ${data.data.user.company?.name || 'GONXT'}`;
        companyText.style.color = '#666';
        companyText.style.marginBottom = '16px';
        
        const roleText = document.createElement('p');
        roleText.textContent = `Role: ${data.data.user.role || 'N/A'}`;
